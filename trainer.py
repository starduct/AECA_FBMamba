...
class StreamingGeospatialDataset(IterableDataset):
    ...
    def stream_chips(self):
        for img_fn, label_fn, group in self.stream_tile_fns():
            ...
            label_fp = rasterio.open(
                label_fn, "r") if self.use_labels else None
            ...
            if self.use_labels:  # garuntee that our label mask has the same dimensions as our imagery
                ...
                print("Image%s: %d x %d" % (img_fn, height, width))
                print("Label%s: %d x %d" % (label_fn, t_height, t_width))
                print("------------------------------------------------")
            ...
            for i in range(self.num_chips_per_tile):
                ...
                if self.windowed_sampling:
                    try:
                        img = np.rollaxis(img_fp.read(window=Window(
                            x, y, self.chip_size, self.chip_size)), 0, 3)
                        # print(img.shape)
                        if self.use_labels:
                            labels = label_fp.read(window=Window(
                                x, y, self.chip_size, self.chip_size)).squeeze()
                    ...
                    if self.use_labels:
                        labels = label_data[y:y +
                                            self.chip_size, x:x+self.chip_size]
...
def trainer_dataset(args, model, snapshot_path):
    ...
    print("The length of train set is: {}".format(
        len(image_fns)*NUM_CHIPS_PER_TILE))
    ...
    for epoch_num in iterator:
        ...
        for i_batch, (image_batch, label_batch) in tqdm(enumerate(trainloader),  total=num_training_batches_per_epoch):
            ...
            # General CE loss for CNN branch
            loss_ce1 = ce_loss(outputs1, label_batch[:].long())
            # Mask CE (mce) loss for ViT branch
            loss_ce2 = ce_loss(outputs2, mask_output[:].long())
        ...
        logging.info('Epoch : %d, CE-branch1 : %f, MCE-branch2: %f, loss: %f' %
                     (epoch_num, avg_loss1, avg_loss2, avg_loss1*0.5+avg_loss2*0.5))
        save_interval = 20
