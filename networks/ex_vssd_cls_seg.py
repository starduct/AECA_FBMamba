...
def segsum(x):
    ...
    x_segsum = x_segsum.masked_fill(~mask, -np.inf)
    return x_segsum
...
class VSSDSegHead(nn.Module):
    def __init__(self, de_base_channels=128, out_channel=1, group=8):
        ...
        self.up0 = nn.Sequential(
            nn.Conv2d(64, de_base_channels, 3, 1, 1), nn.AvgPool2d(2, 2))
        self.up1 = nn.Sequential(nn.Conv2d(128, de_base_channels, 3, 1, 1), )
        self.up2 = nn.Sequential(nn.Conv2d(
            256, de_base_channels, 3, 1, 1), nn.UpsamplingBilinear2d(scale_factor=2), )
        self.up3 = nn.Sequential(nn.Conv2d(
            512, de_base_channels, 3, 1, 1), nn.UpsamplingBilinear2d(scale_factor=4), )
...
class VSSDClsHead(nn.Module):
    def __init__(self, de_base_channels=128, out_channel=1, group=8):
        ...
        # 将vssd的输出通道数都改成 de_base_channels
        self.down0 = nn.Sequential(AdaptiveMaxPool2d(
            (4, 4)), nn.Conv2d(64,  de_base_channels, 3, 1, 1))
        self.down1 = nn.Sequential(AdaptiveMaxPool2d(
            (4, 4)), nn.Conv2d(128, de_base_channels, 3, 1, 1))
        self.down2 = nn.Sequential(AdaptiveMaxPool2d(
            (4, 4)), nn.Conv2d(256, de_base_channels, 3, 1, 1))
        self.down3 = nn.Sequential(AdaptiveMaxPool2d(
            (4, 4)), nn.Conv2d(512, de_base_channels, 3, 1, 1))
...
class SegVSSD(nn.Module):
    def __init__(self, fact=(2, 2), in_chans=3, out_channel=1, group=8):
        ...
        self.head = VSSDSegHead(
            128, out_channel * fact[0] * fact[1], group=group)

    def forward(self, x):
        b, c, H, W = x.shape
        h, fh = H // self.fh,  self.fh
        w, fw = W // self.fw,  self.fw
        x = x.reshape(b, c, h, fh, w, fw).permute(
            0, 1, 3, 5, 2, 4).reshape(b, c*fh*fw, h, w)
        y = self.head(self.vssd(x))
        y = y.reshape(b, -1, fh, fw, h, w) .permute(0,
                                                    1, 4, 2, 5, 3).reshape(b, -1, H, W)
        return y
...
if __name__ == '__main__':
    ...
    block = VMAMBA2Block(dim=768,
                         input_resolution=(14, 14),
                         num_heads=8,
                         linear_attn_duality=False,
                         ssd_chunk_size=14)
    x1 = torch.randn(10, 196, 768)

    output = block(x1, 14, 14)
    print(block)
